...
### 1.1 Correct SPARK_BASELINE.md `Current Config` section ⏸ DEFERRED — awaiting user confirmation
**Status:** BLOCKED (awaiting user OK — constitution reserves `Current Config` to the user; see U-7. Exact diff handed to user 2026-06-15.)
**Model Tier:** haiku
**Depends On:** none
...
### 3.1 Back up and extend the GPU exporter
**Status:** PENDING
...
### 3.2 Add Grafana alert rules (new objects only)
**Status:** PENDING
**Execution Mode:** GUIDED-SSH
**Depends On:** 3.1 + Prometheus scrape confirmed
**Tasks (wget API, datasource UID `PBFA97CFB590B2093`, never touch existing dashboards):** read actual vLLM/exporter job labels from Prometheus first, then create 3 rules — `increase(gpu_xid_events_total[15m]) > 0` (critical), qwen35 restart `changes(process_start_time_seconds{<job>}[30m]) > 0` (warning), vLLM scrape `up == 0` (service down). Route per the alert-notification decision (see Unknowns U-2).
**Acceptance Criteria:** 3 rules exist with unique UIDs; a test-fire reaches the chosen destination; no existing dashboard modified.
...
### 4.1 Day-of pre-flight
**Status:** PENDING
**Execution Mode:** GUIDED-SSH
**Tasks:** confirm idle (0 running/waiting); re-verify dkms empty + `dpkg --audit` clean; `apt -s dist-upgrade` reviewed for removals/holds; `dpkg -l > /home/claude/pre-upgrade-dpkg.txt`; confirm compose backups present; **obtain explicit physical-console confirmation from user.**
**Acceptance Criteria:** all green; user confirms physical presence.

### 4.2 Execute upgrade + reboot
**Status:** PENDING
**Execution Mode:** PHYSICAL
**Depends On:** 4.1
**Tasks:** `sudo apt update && sudo apt dist-upgrade` → **verify/install `linux-modules-nvidia-580-open-6.17.0-1021`** (per firmware-kernel rule; signed prebuilt) → `fwupdmgr refresh && fwupdmgr get-updates` (expect none) → reboot with user watching.
**Acceptance Criteria:** clean reboot; system back up.
...
**Depends On:** 4.3
**Tasks:** `run_full_suite.sh current_baseline` (bench stages: throughput c1/4/8/16 ×3 + AR 30); write Entry 077; `restore_production.sh`.
...
- [ ] Re-baseline recorded (Entry 077); production restored
...
**Acceptance Criteria:** throughput vs Entry 077; AR ≥28/30; DFlash acceptance recorded; zero Xid/restart in soak.
...
**Acceptance Criteria:** build effect measured vs Entry 077; pre-quant-vs-on-the-fly verdict re-confirmed for 0.22; soak stability.
...
**Adoption gates (ALL):** ≥+5% c8 vs Entry 077 re-baseline; AR ≥28/30; 12h soak zero Xid/restart. Then present to user. On approval: show one-param compose diff first; `.pre-<change>` backup; staged restart per pre-flight; Entry + CLAUDE.md updates. `restore_production.sh` after every session regardless.
...
- [ ] All three adoption gates evaluated against Entry 077
...
| Exporter breakage | 3 | Low | `.bak-20260611` + immediate `curl` verify | Open |
| Touching a user Grafana dashboard | 3 | High | New UIDs only; never PUT existing dashboards | Open |
| Arena +20% not portable to shared-GPU stack (we run gpu_util 0.70 / 3 models vs Arena solo 0.85) | 5 | Low | Treat +20% as ceiling; gate on +5% c8 floor vs Entry 077 | Open |
...
| U-2 | Alert-notification routing (email/push vs dashboard-only) | Low | 3.2 | **User decision needed before 3.2** |
...
| U-7 | `Current Config` correction approval | Low | 1.1 | **User confirm to apply** |
