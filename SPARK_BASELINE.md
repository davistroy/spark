...
Last updated: 2026-06-30 (Entries 081-082 — spark-recon + spark-audit)
Last recon: 2026-06-30 (Entry 081)
Last audit: 2026-06-30 (Entry 082)
...
| arena_top_nvfp4_vllm_tok_s | **118.91 on PORTABLE vLLM** (Luis Poveda, `nvidia/Qwen3.6-35B-A3B-NVFP4`, Marlin MoE + FULL CUDA graphs/no enforce-eager + FlashInfer attn + MTP=3 + FP8 KV; container `vllm-node`). Submitted 2026-06-30. **+78% vs prod 66.9, +48% vs best FP8-vLLM 80.27, +54% vs prior NVFP4-vLLM ceiling (Frick 77.07).** Corroborated by Hon Lam Gabriel Leung 76.81 (06-27). Challenges the "no 4-bit path on SM121" rule (that was AWQ-INT4; NVFP4 = native Blackwell FP4). Captured 2026-06-30 (Entry 081) |
...
| Field | Value |
|-------|-------|
| vllm_last_checked_version | v0.24.0 (stable 2026-06-29; prior v0.23.0 2026-06-15) |
| vllm_latest_observed | v0.24.0 (stable 2026-06-29, WORTH WATCHING) — newer than v0.23.0; HIGH-by-keyword but **SM120 ≠ our SM121** (DeepSeek-V4 desktop-Blackwell #43477; no SM121/GB10/#38126/cuDNN text). DFlash now mainline (#44586, DFlash+FlashInfer #43081); EAGLE3-for-Qwen3 #43132. **Gemma4 #39138/#40099 + DeepGEMM SM12.x #41063 ALL STILL OPEN** (no movement since 06-16) → DeepGEMM-FP8 bench stays gated off. Arm C build-upgrade candidate #2. Prior: v0.23.0 (2026-06-15, WORTH WATCHING). 408 commits / 200 contributors. Highlights: DeepSeek-V4 hardening, Model Runner V2 default for Llama/Mistral dense, Gemma 4 Unified encoder-free, multi-tier KV cache offloading, "breakable CUDA graphs," prefix-cache corruption remedies, Rust frontend expansion. **No explicit SM121/GB10 text in release notes** — prior belief that cuDNN graph-corruption bug is fixed in v0.23.0 not confirmed from release notes (may still be true; verify via soak test). DeepGEMM SM12x still blocked (#41063 open). Gemma4 PRs #39138 AND #40099 BOTH still open. **This is the target version for upgrade eval** (eugr already tracking 0.23.1rc1.dev53 same day, 2026-06-16). Prior: v0.22.0 (2026-05-29) had SM121 kernel work (#41215 per-tensor FP8 CUTLASS, #40082 FlashInfer MoE SM120/121, #40717 Blackwell GDN prefill). |
| qwen_current_model | Qwen/Qwen3.6-35B-A3B (adopted 2026-04-23) |
| gemma4_pr_status | #39138 OPEN (needs-rebase as of 2026-06-15; author has not yet rebased) / #40099 OPEN (awaiting code-owner review). Both still required; re-verified unmerged 2026-06-16 (v0.23.0). |

## spark-vllm-docker Tracking
| Field | Value |
|-------|-------|
| svd_last_checked_date | 2026-06-30 (wheels rebuilt 2026-06-28: vLLM 0.23.1rc1.dev537+g6eb63a1da, FlashInfer 0.6.13-5f2bdc41-d20260628; **NEW `qwen3.6-35b-a3b-nvfp4.yaml`** = Marlin MoE + FlashInfer + MTP=3 + FP8-KV (`VLLM_MARLIN_USE_ATOMIC_ADD=1`), defaults TP=2/multi-Spark → needs TP=1 adaptation for single-Spark eval; DFlash recipe now n=15+flash_attn. #164 FlashInfer-missing de-risked but verify for NVFP4 arm. Prior 2026-06-16 (wheels rebuilt 2026-06-16: vLLM **0.23.1rc1.dev53+gc69c73418.d20260616**, FlashInfer **0.6.13-38feb62b-d20260615** — eugr already tracking v0.23.x base same day as v0.23.0 stable. **New: DFlash + FlashInfer FP8 KV Cache recipe** — enables DFlash without BF16 KV memory overhead, directly addressing the KV-budget concern from Entry 073. Caution: spark-arena/sparkrun issue #164 reports eugr `:latest` rebuild path may omit FlashInfer — verify image has FlashInfer before DFlash+FP8-KV eval. Prior (2026-06-11): vLLM 0.22.1rc1.dev330, FlashInfer 0.6.13.) |

## Forum Tracking
| Field | Value |
|-------|-------|
| forum_last_checked_date | 2026-06-30 (719/721/723 all accessible. **NEW: CVE-2026-24218** shared/cloned-SSH-host-key RCE on DGX OS — Spark CONFIRMED likely-affected (host keys mtime 2025-09-22 pre-dates fs install, `root@localhost` comment, never regenerated), remediation pending. NVFP4 SM121 frontier corroborated (iromu 249-268 tok/s, 92% accept). vLLM 0.23.x confirmed on GB10 (eugr). No new driver/kernel/firmware (current 580.159.03/1021). #37754 still open, we're unaffected (FLASH_ATTN). Prior 2026-06-16 partial — 719.json 403) |
| forum_endpoints | **Category 720 PERMANENTLY REMOVED** (404 re-confirmed; /c/721/show.json shows 719 parent + 721 only child; former projects topics merged in). Scan 719.json only — it aggregates everything; 721 is a near-duplicate. **Category 723 (`dgx-spark-gb10-projects`) observed in search results** — may be projects sub-category replacement for removed 720; verify next run. Endpoint list fix needed in spark-recon skill source. |
...
## Watch Items
- **[ACTION 2026-06-30 — Entry 081] SECURITY: CVE-2026-24218 — DGX OS shared/cloned SSH host keys → unauth network RCE/impersonation.** Spark CONFIRMED likely-affected: host keys mtime 2025-09-22 23:46 (pre-dates root-fs birth 23:49 and installer 00:05), `root@localhost` comment, never regenerated → factory-image keys shared across units. Remediation (interactive sudo by davistroy; changes host fingerprint, coordinate to avoid lockout): `sudo rm /etc/ssh/ssh_host_* && sudo ssh-keygen -A && sudo systemctl restart ssh`, then re-pin in local `~/.ssh/known_hosts`. **Not yet applied.**
- **[WATCH — BLOCKED on v0.23.x build; opportunity Entry 081, B1 result Entry 083] NVFP4-on-vLLM eval — coupled to Arm C (v0.23.x build upgrade).** **B1 (2026-06-30) ran NVFP4 on the CURRENT build → FAILED to load:** `KeyError 'layers.0.mlp.experts.w2_input_scale'` (`qwen3_5.py:407`) — current loader lacks NVFP4 MoE expert-scale mapping. **NOT a kernel/#2776/Marlin issue; no flag fix.** Confirms NVFP4 needs v0.23.x. **Opportunity remains strong:** Arena Poveda `nvidia/Qwen3.6-35B-A3B-NVFP4` **118.91 tok/s c1 (+78% vs prod)** on portable vLLM (Marlin MoE + full CUDA graphs); eugr `qwen3.6-35b-a3b-nvfp4.yaml`; forum iromu 249-268 tok/s @ 92% accept. **Next step = B2: build a v0.23.x SM121 image** (`git pull /home/claude/spark-vllm-docker` + build — NOT pullable; multi-hour + memory pressure on a box holding ~102 GB models → run with prod down or limited BUILD_JOBS). Then B3 (faithful NVFP4 suite, TP=1, FP8 KV, MTP=3, Marlin MoE, FlashInfer attn; verify FlashInfer #164; watch #2776) + B4 (FP8/MTP=2 build control on same image, = Arm C) + gate **≥+5% c8 vs 406.9** + AR≥28/30 + soak. NVFP4 model already cached (22 GB). **Deferred to a user-scheduled build window.** Sandbox only; eval ≠ adoption (Entry 073 precedent). Still challenges the "no viable 4-bit path on SM121" rule (that was AWQ-INT4 gs=32; NVFP4 is native FP4) — pending the build.
...
- **[ESCALATED 2026-06-16 — v0.23.0 stable + eugr wheels ready; DFlash+FP8-KV available]** **vLLM 0.23.x upgrade eval: all gating conditions met.** v0.23.0 released 2026-06-15 (408 commits, 200 contributors); eugr tracking v0.23.1rc1.dev53 same day. **New in eugr: DFlash + FlashInfer FP8 KV Cache** recipe — eliminates the BF16 KV budget penalty (no -55% token regression). No explicit SM121/cuDNN text confirmed in v0.23.0 release notes; verify cuDNN graph fix via soak test. v0.22.0 = first stable with SM121 kernels (#41215 FP8 CUTLASS, #40082 FlashInfer MoE, #40717 Blackwell GDN). Migration notes unchanged: `--speculative-config` JSON, gpu_mem_util→0.8, validate UMA accounting. Caution: sparkrun issue #164 (FlashInfer missing from eugr :latest rebuild path) — verify before DFlash eval. Schedule Phase 5 eval window (multi-hour, needs idle prod + supervision). Sandbox only.
