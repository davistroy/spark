...
2. **Update the memory file** — write a detailed entry in `<user-home>\.claude\projects\C--Users-Troy-Davis-dev-personal-spark\memory\` in the appropriate topic file.
...
- **HF cache owned by root:** `/home/<user>/.cache/huggingface` is root-owned (from vLLM docker volume mounts). Non-root processes must use a separate cache dir or set `HF_HOME` to a user-writable location.
...
- **Primary user has no passwordless sudo:** OS-level changes (sysctl, systemctl) require interactive sudo password entry — cannot be done via non-interactive SSH from remote Claude sessions.
...
- **HF cache absolute path required:** Docker volume mounts must use `/home/<user>/.cache/huggingface`, NOT `~/.cache/huggingface`. The `claude` user's `~` expands to `/home/claude` which has no model weights. Using the wrong path causes silent hang (container tries to download 35GB model).
...
- **FP8 MoE backend (v0.19.0):** TRITON auto-selected for FP8 MoE, CUTLASS for FP8 linear ops. Both have native SM121 support. `VLLM_TEST_FORCE_FP8_MARLIN=1` is NO LONGER NEEDED — removed from production config on 2026-04-11. Marlin tested at ~3% difference (within noise) but shows "no native FP8" warning. Auto-select is the correct path.
- **Pre-quantized FP8 model hangs on v0.19.0:** `Qwen/Qwen3.5-35B-A3B-FP8` (pre-quantized checkpoint) causes vLLM v0.19.0 to hang during model loading with 0% GPU activity. Use on-the-fly FP8 quantization (`--quantization fp8` with `Qwen/Qwen3.5-35B-A3B`) instead.
- **Ethernet static IP configured:** enP7s7 set to <spark-lan-ip>/24 (manual, gateway <gateway-ip>), route-metric=700 (WiFi=600 takes priority). WiFi stays on <spark-lan-ip>. NM connection files stored in both `/run/NetworkManager/system-connections/` (runtime) and `/etc/NetworkManager/system-connections/` (persistent). NM on this system reads from `/run/` first — always write there AND `/etc/`. NM auto-generates volatile `enP7s7.nmconnection` profiles that override our "Wired connection 3" — delete them and restart NM.
- **Ethernet switch port: USW Pro 24 Port 10.** Do NOT move the cable between switch ports. The USW Pro 24 retains stale MAC table entries for disconnected ports and triggers MAC flapping detection, silently dropping all frames from the MAC. If you must change ports, reboot the switch from the UniFi controller afterward.
- **Dual-homed routing hazard:** WiFi (.32) and Ethernet (.33) on the same 192.168.10.0/24 subnet. Ethernet route metric MUST be higher than WiFi (700 vs 600). If ethernet is broken and has a lower metric, ALL LAN traffic fails — including WiFi responses.
...
2. **Diff before execute.** If changing any parameter:
   ...
   - Verify HF cache path is `/home/<user>/.cache/huggingface` (not `/home/claude/...`)
