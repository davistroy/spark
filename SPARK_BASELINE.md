...
- **[UPDATED 2026-07-05 — Entry 099]** **Qwen3.7 27B/35B open weights NOT released — 47 days past Qwen3.7-Max API launch (May 19). July 16 deadline in 11 days.** Three consecutive post-3.6 Qwen releases (3.6-Plus, 3.7-Max, 3.7-Plus) are all API-only, closed-weight. No HF repo under official Qwen org. Realistic window now mid-July only; probability declining. Holo-3.1-35B-A3B open weights confirmed (Entry 099) but it is a VLM — not a general LLM successor. Ornith-1.0-35B-FP8 remains best confirmed open-weight A3B general LLM alternative (MTP pre-screen required). A Qwen3.7-35B-A3B-FP8 would still be a near-drop-in successor when released. Beware HF squat `RscriptSQuen/Qwen3.7-plus` (fake). **If absent July 16, shift to closed-weight-first conclusion.**
