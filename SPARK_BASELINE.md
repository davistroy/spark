...
| Field | Value |
|-------|-------|
| svd_last_checked_date | 2026-07-02 (Entry 096). **NEW BUILD: dev701 (`0.23.1rc1.dev701+g00eb7cefa.d20260701`, July 1 18:06 UTC) + FlashInfer `0.6.14-8fc7f079-d20260701` (18:04 UTC)** — FlashInfer MINOR VERSION BUMP from 0.6.13 → 0.6.14. dev537→dev701 = +164 commits in ~3 days. Still v0.23.1rc1 base (NOT v0.24.x-based). **Arm C eval target: dev701 + FlashInfer 0.6.14** (supersedes dev537). New July 1 commits: (a) NVFP4 regression fix — Dockerfile reverts vLLM PR #46756 (corrupts NVFP4 gen for Qwen3.6-35B-A3B-NVFP4/Nemotron-120B-NVFP4 in v0.24.0; conditional revert applied); (b) tf5 flag/image deprecated. June 29 commits: Gemma4-26B recipe with MTP; NCCL→main branch; Rust frontend support. DSV4F recipe (2-node only) added June 20-25. PR #279 (DFlash + FP8 KV Cache): still OPEN. Prior dev537 (`0.23.1rc1.dev537+g6eb63a1da.d20260628`, June 28, FlashInfer `0.6.13-5f2bdc41-d20260628`) superseded. Fallback pin: dev448 (`0.23.1rc1.dev448+ge53a17232.d20260625`, FlashInfer `0.6.13-25dd814e-d20260625`). All other prior notes unchanged: gpu_memory_utilization default 0.7→0.8; CUDA base `nvidia/cuda:13.0.2-devel-ubuntu24.04`; MiniMax IPC fused path disabled (PR #43410); do not combine instanttensor with DFlash (issue #211). |

## Forum Tracking
| Field | Value |
|-------|-------|
| forum_last_checked_date | 2026-07-03 (Entry 097; WebSearch fallback; 719.json returns 403 in remote execution env) |
...
- **[UPDATED 2026-07-03 — Entry 097] DGX Spark hardware-reliability cluster — now 6 threads; thermal root cause confirmed; community workaround found.** /t/373251 (hard power-off within 60s of vLLM stress test, latest OTA/580.x, reproducible on demand); /t/362483 "System powers off" (older); /t/372089 "DGX Spark Failure – Unable to Power On" (Entry 087); /t/372486 "DGX Spark GB10 – Asus GX10 – GPU becomes inoperable" (Entry 088; GPU enters non-functional state after repeated workloads, Asus-specific); /t/372469 (MODS-020000600139 hard-power-off); **/t/375158 "Air conditioned GB10 x 2 — How I stopped sudden shutdowns"** (July 1, 2026 — THERMAL SOLUTION: room A/C eliminates shutdowns for dual-GB10; also /t/369457 RMA requested). Thermal root cause confirmed. **Mitigation stack (community-validated):** (1) room A/C (/t/375158); (2) GPU clock cap 2200 MHz; (3) ASUS v0103 firmware (−8–10°C). Also: fan headless-mode bug on some GB10 units (fans may not spin in headless boot — verify on production unit). No NVIDIA resolution for MODS-020000600139. Our production unit: 45+ days clean, zero restarts/OOM. Low urgency — monitor.
