...
- **[ACTION 2026-06-30 — Entry 081] NVFP4-on-vLLM = top portable perf lead — schedule Phase 5 eval arm.** 3-source cross-correlation: Arena Poveda 118.91 tok/s c1 (+78% vs prod) on portable vLLM w/ Marlin MoE + FULL CUDA graphs; eugr `qwen3.6-35b-a3b-nvfp4.yaml` recipe; forum iromu 249-268 tok/s @ 92% accept. **Challenges verified rule "no viable 4-bit path on SM121 / Marlin WNA16 hangs"** (that was AWQ-INT4 gs=32; NVFP4 = native Blackwell FP4, runs full CUDA graphs). Eval: adapt recipe TP=2→TP=1 + raise gpu-util, keep FP8 KV + MTP=3 + Marlin MoE + FlashInfer attn; verify FlashInfer in image (#164); watch flashinfer #2776 (NVFP4-MoE graph-capture crash; build-dependent). Gate ≥+5% c8 vs MTP baseline 406.9 (Entry 080). Pairs with Arm C (eugr 0.23.x image ships the recipe). Sandbox only — do NOT touch production qwen35.
