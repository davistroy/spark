...
**[ADDENDUM — Arena agent results, post-commit]**

Arena subagent returned additional findings after Entry 142 was already committed:

**A. New Qwen3.6-35B-A3B-FP8 vLLM submission (tg128 c1 metric UNKNOWN — trigger UNCONFIRMED):** @spark_arena announced "Qwen/Qwen3.6-35B-A3B-FP8 achieved 130 tokens/sec on text generation on NVIDIA DGX Spark with vLLM at concurrency 10, for a 128 tokens reply with 100k tokens of prior context already in memory." The linked Arena submission uses `ghcr.io/spark-arena/dgx-vllm-eugr-nightly:latest` with a substantially different config vs Stojanovic's baseline: `--kv-cache-dtype fp8` (FP8 KV cache), `--attention-backend flashinfer` (not FLASH_ATTN), `--load-format instanttensor`, `--max-model-len 262144` (2× production 131072), `--gpu-memory-utilization 0.8` (vs 0.70 production), MTP n=2. The "130 tok/s at c10" is NOT the tracked tg128 c1 metric — the aggregate Arena score and actual c1 number are unknown (submission ID not retrievable; spark-arena.com egress-blocked). **10% trigger (>88.30 tok/s tg128 c1) CANNOT be confirmed or cleared without direct access.** Stojanovic's recipeCopyCount unchanged at 205 suggests the community has not been copying his recipe since the new submission appeared. **Recommended: check spark-arena.com from the Spark device to retrieve the new submission's tg128 c1 score and aggregate.**

**B. NVIDIA Forum /t/379875 MISSED in Entry 141:** Thread "Benchmarking NVIDIA Nemotron 3.5 Lightning 30B-A3B-NVFP4 on DGX Spark" — between Entry 140 threshold /t/379627 and Entry 141 threshold /t/379959, therefore should have appeared in Entry 141 Check 5 but was missed. Reported numbers: 116.84 tok/s (unknown submitter), 125.13 tok/s (SaiyamPathak), 200+ tok/s claimed (Jaime Aita). NOTE: These are Nemotron 3.5 Lightning NVFP4 30B/3B-active — different model class from production 35B FP8. The 200+ tok/s at unknown concurrency/context is unverified. Consistent with Entry 141 Watch Item community numbers (108–116.84 tok/s c1). The 125.13 and 200+ claims need verification — may be at higher concurrency. Thread content blocked (403), so details unverifiable from here.

**C. Qwen3.8-27B official recipe confirmed:** `official-recipes/qwen3.8/qwen3.8-27b-fp8-mtp-vllm.yaml` exists in recipe registry. No benchmark scores attached yet — Arena submissions pending community testing.
