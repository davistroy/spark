...
- **DFlash spec-decode REJECTED for production on SM121 (Entry 080, 2026-06-16):** drafter `z-lab/Qwen3.6-35B-A3B-DFlash` n=8 loads natively on `vllm-cu132-test:latest` (needs `--trust-remote-code`; arch `DFlashDraftModel`) — but it is a **single-stream latency optimizer, not a throughput one.** Gate-valid head-to-head vs the production MTP=2 config (both on kernel 1021, harness 600-tok×3): c1 **+6.3%** (77.7 vs 73.1), c4 −2%, c8 **−16.8%** (338.4 vs 406.9), c16 **−42.2%** (421.9 vs 730.5). 37.2% token acceptance on the 8-wide draft (~3 of 8 accepted) vs MTP's 79.8% on a 2-wide draft. Quality parity (AR 28/30, same fixtures). Fails the ≥+5% c8 adoption gate decisively. **Keep MTP=2** for the shared endpoint; only reconsider DFlash if the workload is proven pure-c1 interactive. Eval harness (`/home/claude/llm-eval/`) gained `LLM_SPEC_METHOD`/`LLM_SPEC_MODEL` profile vars; also fixed a compose `${VAR:-default}` bug (colon-dash) that injected a spurious `--quantization fp8` onto the pre-quant model — eval profiles must use the no-colon `${VAR-default}` form so an explicit empty is honored (backups `*.bak-20260616`).
- **NVFP4 (modelopt-FP4) does NOT load on the current cu132 build (v0.19.1rc1.dev219) — Entry 094, 2026-06-30:** `nvidia/Qwen3.6-35B-A3B-NVFP4` dies at weight-load with `KeyError: 'layers.0.mlp.experts.w2_input_scale'` (`vllm/model_executor/models/qwen3_5.py:407`) — the loader has no mapping for the NVFP4 MoE per-expert input-scale tensors. **NOT a kernel/CUDA-graph/#2776 issue and NOT a Marlin hang** — a pure weight-schema gap that no runtime flag fixes. **The NVFP4 eval is therefore coupled to the v0.23.x/v0.24.0 build upgrade (Arm C); there is no current-build shortcut.** Model IS cached (22 GB, `quant_method: modelopt`, arch `Qwen3_5MoeForConditionalGeneration`). External Arena NVFP4 wins (Poveda `nvidia/Qwen3.6-35B-A3B-NVFP4` 118.91 tok/s c1, +78% vs prod) all ran on newer builds — consistent. The B1 probe (one ~12-min idle prod-down window, zero GPU risk) was the correct cheap test before committing to the multi-hour build. **Distinct from the AWQ-INT4 rule** — that was a CUDA-graph hang; this is a load-time schema gap.
- **CVE-2026-24218 — Spark SSH host keys ROTATED 2026-06-30 (Entry 094):** factory image shipped shared/cloned host keys (mtime predated fs-install). Regenerated via claude NOPASSWD primitives (`ssh-keygen` as user → `sudo cp/chown/chmod` into `/etc/ssh` → `sudo systemctl restart ssh`; no davistroy sudo needed). **New ED25519 fingerprint: `SHA256:xXNusbupisxTmURNJV5khmepwIoym0UldLz3020g14c`** (ecdsa `lQ9gqI8…`, rsa `vEjO1Ydc…`). Old factory keys backed up at `/home/claude/ssh-hostkey-backup-20260630/`. Re-pin `known_hosts` on any client that connected before this date.
