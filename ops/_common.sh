...
# public repo. Use `:=` form inside these files (e.g. `: "${SPARK_PUSHOVER_USER:=KEY}"`)
...
# Alert delivery: Pushover only (SPARK_PUSHOVER_TOKEN + SPARK_PUSHOVER_USER). If those
# are empty, notify() resolves them from Bitwarden at runtime (see
# _spark_resolve_pushover). Empty = log-only, and notify() says so.
#
# The generic SPARK_ALERT_WEBHOOK raw-body fallback was REMOVED 2026-07-17: it was
# never set anywhere, and it was a re-entry point for exactly the ad-hoc channels the
# fleet standardised away from (notification audit D1 — Pushover/Slack/email only).
...
notify() {
  ...
  if [ -z "$via" ]; then
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [$routine] NO CHANNEL DELIVERED for [$sev] — alert is log-only" \
