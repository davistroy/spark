...
## Entry 116 — Spark unreachable: SSH banner + LLM + cross-encoder dead, embedding still up (2026-08-03)

**Symptom reported (Troy, remote-diagnosis request).** Embedding service reachable; vLLM (`:8000`) and ce-service (`:8005`) unreachable; SSH itself fails to return a banner (TCP connects, nothing answers). Troy's own read: pattern looks like host-level thrashing under load, not a firewall block. Diagnosed remotely from ubuntu-vm — **no live SSH/console access to the box during this session**, so this is evidence-based inference, not a confirmed root cause.

**Evidence gathered (read-only, no writes to the box):**
1. **This VM cannot reach Spark at all**, on any path: `ping` 100% loss; `curl` to 8000-8005 all failed outright (no response, not even a non-200); `tailscale ping spark` timed out.
2. **Tailscale peer state is the key signal:** `tailscale status --json` for `spark` shows `RxBytes: 0`, `LastHandshake: 0001-01-01T00:00:00Z` (never completed this epoch), `Relay: "mia"` (no direct path), and **`LastSeen: 2026-08-03T07:00:46Z`** — Tailscale's control plane hasn't heard from Spark's tailscaled since ~03:00 EDT today, i.e. **~9h of total silence** as of this check (12:19 EDT). This is a much longer and more total outage than "app is slow to answer" — it reads as the whole host (or at minimum its network stack) has been down/hung since ~3am.
3. **Cross-checked against the daily healthcheck cron** (`~/spark-ops-logs/healthcheck.log`, runs 13:05 EDT via SSH from this VM): last successful run was **yesterday 2026-08-02 17:05 UTC (13:05 EDT)** — SSH worked fine then, GPU was at `temp=63C util=90%` (heavy load), only pre-existing anomaly was `bge-m3` unhealthy. So the failure onset is somewhere between 13:05 EDT yesterday and ~03:00 EDT today. Today's 13:05 EDT run hadn't fired yet at diagnosis time (cron fires ~45min after this check) — `spark-healthcheck.sh` explicitly distinguishes this failure mode (`if SSH_RC -ne 0: notify CRIT "SSH/gather failed — box unreachable or docker down"`), so Troy should get a distinct Pushover CRIT (not a generic anomaly) if it's still down at 13:05 EDT.
4. **Historical pattern strongly supports a resource-exhaustion narrative, not a network/firewall issue:**
   - 2026-07-21 to 23: swap climbed past the 12GB warn threshold three days running (13.85→14.03GB), with a `qwen35` restart on the 21st.
   - 2026-07-24: **Entry 115** deliberately stopped `bge-m3`/`ce-service`/`gliner` (freed ~25GB) to unblock the CS-R R4 ChromaDB reload, and trimmed `qwen35` `--gpu-memory-utilization` 0.70→0.60. The **"Restore TODO" for `bge-m3` was never completed** — it's shown "unhealthy" in every single daily healthcheck from 07-24 through 08-02 (10 straight days). `ce-service`/`gliner` *were* restored at some point (they drop out of the unhealthy list after 07-24) — but `ce-service` being unreachable today means it's down *again*, a fresh event on top of the still-unfixed `bge-m3`.
   - 2026-07-29: a much bigger event — only 3/8 containers running, everything else (`qwen35`,`qwen3-embed`,`gliner`,`chromadb`,`neo4j`,`bge-m3`,`ce-service`) unhealthy simultaneously, swap crashed to 936MB (consistent with a full stack restart/recovery). Recovered by 07-30 with swap near-zero.
   - **Today's event is worse than any of the above:** every prior anomaly (including the 07-29 near-total outage) still had a working SSH session for the healthcheck to run over. Today, SSH itself gives no banner and Tailscale shows zero response for 9+ hours — that's consistent with the host being far enough into memory/swap thrashing (or fully hung/crashed) that even sshd can't fork a handler, not just individual containers crash-looping.
5. **Asymmetry to note:** Troy reports embedding still answers. If that check is via LAN-direct (`192.168.10.32:8001`) rather than Tailscale, it doesn't contradict the thrashing theory — kernel-level TCP SYN/ACK is cheap and can still complete even when userspace is starved for scheduling, while forking a new sshd session or servicing a fresh vLLM/ce-service request needs actual CPU time the box may not be able to spare. An already-running embedding worker with warm threads could keep answering while new-process-spawning paths (SSH, cold requests to the heavier services) starve. Worth confirming which path Troy used for the embedding check.

**Not done (deliberately):** No remote remediation attempted — I have no reachable path to the box at all right now, and even if I did, blind restart/reboot of a possibly-thrashing or hung host is exactly the "STOP, confirm physical console access" case in CLAUDE.md. **Remote power-cycle is not available as a fallback** — the Shelly Plug (ordered 2026-04-13, see `shelly-power-switch.md`) was never actually set up (no static IP assigned, no post-arrival steps completed) — if this turns out to be a true hang, recovery needs physical presence.

**Recommended next steps for Troy:**
- Confirm which path the "embedding is fine" check used (LAN-direct vs Tailscale) — resolves the asymmetry above.
- Wait for the 13:05 EDT healthcheck cron (~45min out at diagnosis time) — it will page distinctly ("SSH/gather failed — box unreachable") if still down, or resolve automatically if the box recovered.
- If confirmed hung: this is a physical-access recovery (power cycle), not a remote fix — no shortcut currently exists.
- Once back up: finish the 10-day-old Entry 115 Restore TODO (`bge-m3`, and consider whether `qwen35` gpu_util should go back to 0.70) — it's the most likely standing contributor to repeat resource-pressure incidents.

**UPDATE — self-recovered, root cause narrowed (2026-08-03, ~12:26 EDT, same session):** Reachability returned on its own (Tailscale direct-path handshake completed, ping clean) before the 13:05 EDT cron fired and before any physical intervention. Live SSH check immediately after recovery found:
- **No reboot** — `uptime` shows 48 days continuous (since 2026-06-15); this was a hang/thrash, not a crash-reboot. Confirms the "host-level thrashing" read, not a firewall/network-path issue.
- **Load average was the smoking gun:** 37.17 (1min) / 52.54 (5min) / 43.29 (15min) at first contact, dropping to 9.10 (1min) within 90 seconds — on this 20-core box that's 2-2.5x oversubscription clearing in real time. `vmstat` showed `wa≈0-1%, id≈99%` moments later — the CPU-starvation spike was over, not ongoing.
- **Memory is still razor-thin:** `free -h` showed 121Gi/121Gi used, only ~100-178Mi "available", and 10Gi/15Gi swap in use — both immediately after recovery and ~90s later. No dmesg OOM-killer or kernel-panic evidence found (ring buffer checked). This looks like severe swap-thrashing (not an actual OOM-kill) — RAM saturated, heavy page-in/out, enough to starve sshd's ability to fork a session and stall the network stack, without ever triggering the kernel OOM killer.
- **The specific trigger: `chromadb` had just restarted** — `RestartCount=1`, `StartedAt=2026-08-03T16:26:11Z` (seconds before first successful ping), **`OOMKilled=false`, `ExitCode=0`** — a clean exit/restart, not a crash. No cron/systemd timer on the box itself restarts it (checked `crontab -l`, `systemctl list-timers`) — the trigger for *that specific restart* is unexplained, but its effect (resetting chromadb's footprint to ~2MB/10GiB from whatever it had grown to) lines up exactly with when the host recovered. Every other tracked container (`qwen35`, `qwen3-embed`, `gliner`, `neo4j`, `ce-service`) has `restarts=0` since the 07-29 crash-recovery — rock stable for 5 days. **`chromadb` is the one recurring troublemaker**, consistent with it holding the largest and still-growing dataset (kb_atoms 138K + kb_resolutions 812K + case_episodes "building" per Entry 115) against its 10GiB cgroup cap.
- **Confirmed still-outstanding from Entry 115:** `qwen35` is still running `--gpu-memory-utilization 0.60` (never reverted to 0.70). `bge-m3` is still `Exited (0) 10 days ago` — clean stop, never restarted.
- **Working theory:** ChromaDB's memory footprint grows over time (index/cache buildup from the ~950K-vector corpus) until, combined with the LLM stack's baseline unified-memory usage, total host RAM saturates and swap thrashing gets severe enough to take down SSH/Tailscale responsiveness system-wide. Something restarts (or crashes and cleanly relaunches) chromadb, which resets its footprint and relieves the pressure — an accidental, undesigned safety valve, not a real fix. The 07-29 near-total-stack crash and today's SSH/Tailscale outage look like two points on the same recurring pattern, at different severities.
- **Not yet done (needs Troy's call, not unilateral action on a stateful/production box):** (2) decide `bge-m3`'s fate — restart now that there's a little headroom, or formally decommission; (3) decide whether to revert `qwen35` to gpu_util 0.70 or leave at 0.60; (4) instrumentation for the growth/leak theory — **installed below.**

**CORRECTION (2026-08-03, ~13:50 EDT, same session) — the "9h Tailscale silence" framing above was wrong; ground truth pulled from Prometheus:** homeserver runs Prometheus scraping `spark-node` (node-exporter, LAN path, not Tailscale) — its `up{job="spark-node"}` series shows the host was **up continuously with zero gaps from before 2026-08-02T11:50Z through 2026-08-03T16:05:59Z** (i.e. through the entire "9h Tailscale silence" window). The Tailscale `LastSeen` staleness (07:00Z) was a **Tailscale-path artifact** (relay/NAT issue between ubuntu-vm and spark specifically, via relay "mia") — **not evidence the host was down or thrashing**. Troy's pushback ("been running fine for days") was correct; my earlier framing overstated it. The LAN-path signal (Prometheus, and presumably Troy's own local checks) is the reliable one going forward — Tailscale-peer staleness alone is not sufficient evidence of host distress.

**Real timeline (Prometheus `node_memory_MemAvailable_bytes{job="spark-node"}`, 2-min resolution):** rock-stable ~9400MB available from 09:51Z through 15:25Z (5.5+ documented hours). **15:31Z (11:31 EDT): sudden cliff — 9400MB→5155MB in one 6-min sample**, continuing to fall (4073→3270→3205→3446MB) through 15:55Z, then a data gap (scrape failures) until the confirmed `up`→`down` transition at **16:05:59Z**. Recovers (scrape succeeds again) at 16:27-16:30Z — matching `chromadb`'s clean restart (`RestartCount 0→1`, `StartedAt 16:26:11Z`, `OOMKilled=false ExitCode=0`) almost to the second — **but available memory never returns to the 9.4GB baseline**, settling instead at 100-200MB continuously through the rest of the session, including during calm periods (load1 as low as 0.31). A **second** `up`→`down` transition hit at **17:30:59Z** (confirmed live via the new memwatch cron and `spark-smoke.sh`'s coincident SSH failure) and **is still down as of this writing** (`up{job="spark-node"}` = 0 at last query).

**The actual trigger — `vllm:num_requests_running{job="vllm-llm"}` (concurrent request count):** baseline was **1** concurrent request for hours (14:52-15:28Z). **15:32Z: jumps to 2, then 15:34Z: 13** — a real burst of concurrent traffic hitting `spark-llm`, landing within 1-3 minutes of the memory cliff at 15:31Z. Burst recedes by 15:36-15:42Z but memory never recovers. **15:58Z: another spike to 16 concurrent**, sustained through **17:04-17:06Z: still 16** — directly overlapping the run-up to the second outage at 17:30Z. Production runs **BF16 KV cache** (not FP8) with a documented **max concurrency of only 3.85x at full 131K context** (spark-device.md) — a burst to 13-16 concurrent requests, if carrying non-trivial context length, plausibly blows straight through that envelope. On GB10's unified memory, KV-cache growth IS host-RAM growth, which is exactly the multi-GB host-memory collapse observed.

**Revised root cause: this looks like a genuine traffic/concurrency event, not a chromadb leak or a multi-day chronic decline.** Something started sending well above the normal 1-concurrent-request baseline to `spark-llm` starting almost exactly at **11:32 EDT today**, with a second burst around **11:58 EDT - 13:06 EDT**. `chromadb`'s restart during the first episode is most plausibly collateral (an OOM-adjacent casualty of overall memory pressure) rather than the instigator the earlier framing assumed. **Open question for Troy: what client/job/pipeline started issuing concurrent requests to spark-llm around 11:30 EDT today** (contact-center-lab / kb-pipeline batch run? an eval? a new integration?) — identifying that answers "why now" definitively and is the actual next step, not further Spark-side infra changes.

**Client-side mechanism identified (2026-08-03, ~14:05 EDT, same session) — Troy confirmed the trigger was his contact-center-lab kb-dedup work.** Explored `~/dev/contact-center-lab`: **Stage 8 (`GlobalDeduplicator`) itself never calls `spark-llm` — only the embedding backend (:8001)**, confirmed via `pipeline/runner.py` grouping stage 8 with `_EMBEDDING_STAGES`, not `_LLM_ONLY_STAGES` (3/6/6a/6b/7). The `spark-llm` traffic came from the shared LLM client (`pipeline/shared/llm_client.py` + `async_llm_processor.py`), used by stages 3/6/6a/6b/7 — so "dedup work" in practice meant a broader pipeline run that includes those LLM stages, not stage 8 in isolation.

**Root mechanism: `pipeline/config.yaml` defines TWO independent client-side concurrency semaphores pointed at the same physical vLLM instance, with no shared coordination:**
```yaml
dgx_spark:  # direct
  base_url: "http://192.168.10.32:8000/v1"
  concurrency: 12
spark:      # same box, via LiteLLM proxy (llm.troy-davis.com)
  concurrency: 32
```
Neither backend's `asyncio.Semaphore` (in `async_llm_processor.py process_batch()`) knows about the other's in-flight requests — `vllm:num_requests_running` (a server-side count) can sum past either individual cap. **This is almost certainly why the observed concurrency (13, then 16) doesn't match either configured limit cleanly**: it's consistent with the direct `dgx_spark` path (concurrency 12) plus a handful of requests routed through the `spark` LiteLLM-proxy path running at the same time. There's already a precedent for this exact class of failure in the repo — `pipeline/runner.py`'s `CONCURRENT_GROUPS` is empty with the comment *"6+7 concurrent group removed — DGX Spark cannot handle both simultaneously (even 16+16 split overloads)"* — but that mitigation only serializes stage 6 vs 7 within one job; it does nothing about two separate backend configs (or two separate job runs) hitting the same server concurrently.

**Compounding factor — Stage 7 batched/document mode holds KV cache for a long time per request:** `stage_7_procedures/run.py` batched mode can request up to `llm_max_tokens_procedures_batched: 8192` tokens per response; the repo's own config.yaml already flagged *"3K-12K token responses ... worst-case 5,254 tokens = 253s ... c16 risks Stage 7 timeout"* — known as a latency risk, but not previously connected to host-memory exhaustion. On unified GB10 memory with BF16 KV cache (**documented max safe concurrency 3.85x at full 131K context**, spark-device.md), a handful of long-held Stage 7 batched requests concurrent with 12+ other in-flight requests plausibly explains both the size (multi-GB) and the persistence (memory pinned, not recovering) of the collapse observed.

**Secondary finding, lower confidence:** `_process_single_inner`'s JSON-parse retry path (higher temperature 0.3 retry on a bad parse) fires a second HTTP call **without re-acquiring the semaphore** — an uncounted concurrency leak where the configured limit doesn't actually bound total in-flight requests during a run with parse failures.

**Recommendations queued for Troy to feed to the pipeline/dedup agent (not yet implemented — client-side change, not Spark-side):** (1) treat `dgx_spark` and `spark` (proxy) as ONE shared budget, never sum past a single safe ceiling — practically, don't run a proxy-routed job concurrently with a direct-backend pipeline run against the same box; (2) lower `dgx_spark.concurrency` well below 12 (the BF16 3.85x envelope is already tight before considering Stage 7's long-held batched responses); (3) add a server-side vLLM `--max-num-seqs` admission-control cap so client misconfiguration can't exceed it regardless of which backend config is used; (4) fix the uncounted parse-retry HTTP call to acquire the semaphore too; (5) consider a pre-flight `/metrics` check (`num_requests_running`+`num_requests_waiting`) before a batch starts, to catch "someone else is already using this instance" that static concurrency alone can't.

**Instrumentation installed (2026-08-03, same session) — `ops/spark-chromadb-memwatch.sh`, cron `*/10 * * * *`:** read-only sampler (docker stats chromadb mem vs 10GiB cap, restart-count delta, host available-mem, swap, load1), logs to `~/spark-ops-logs/chromadb-memwatch.log` **every sample** (not quiet-on-healthy, unlike the other routines — the point is the growth curve, not just anomalies), alerts (Pushover CRIT/ANOMALY via the existing `notify()`) on: SSH failure, chromadb `RestartCount` incrementing again, host available-mem <500MB, or swap ≥12GB. Installed via `ops/install-cron.sh` (idempotent spark-ops crontab block). **First manual run (17:04:45Z, ~38min after recovery) already tripped the available-mem alert: `avail=115MB`** — despite load average and reachability having fully recovered, host free memory is still critically thin (swap still 10.77GB). This confirms available-mem is a much more sensitive leading indicator than the load-average/swap thresholds the daily healthcheck uses — the system is sitting on a knife's edge even in its "recovered" state. Marked TEMPORARY in both the script header and the cron comment — remove once the growth pattern is confirmed/refuted and addressed. Cross-ref [[spark-outage-2026-08-03]].

## Entry 117 — Resilience recommendations: prevent memory pressure from taking down the whole host (2026-08-03)

**Context.** Follow-on from Entry 116. Troy asked: shouldn't Spark fail gracefully under this kind of load instead of taking SSH/Tailscale/the whole host down? **DRAFT ONLY — nothing below has been applied.** Core design gap: there is currently no admission control anywhere in the stack, and nothing protects management-plane processes (sshd, tailscaled, dockerd) from host-wide memory pressure. A single service's overcommit becomes a total-host outage instead of a contained, visible failure. Five independent levers, in suggested rollout order (each is separately revertible):

**1. `--max-num-seqs` on `qwen35` (server-side admission control) — do first, lowest risk/highest leverage.**
Add an explicit `--max-num-seqs N` to the vLLM command in `docker-compose.yml` (currently unset — vLLM defaults high, e.g. 256, and the documented "3.85x max concurrency at 131K context" in spark-device.md is a *theoretical* KV-cache-capacity number, not an enforced scheduler cap). Starting value: pick something at or below that 3.85x figure (e.g. 4-6), rounded down conservatively since Stage 7 batched requests hold KV cache for ~250s each. **Caveat, stated plainly:** it isn't fully confirmed whether the host-RAM blowup in Entry 116 was GPU-KV-cache overcommit specifically, or host-side async/bookkeeping overhead scaling with concurrent+queued request count (prompt/response buffers, tokenizer arrays) — likely both to some degree. Either way, capping `max-num-seqs` bounds the number of requests vLLM will actively track and run, which bounds both. **Verify, don't just assume:** after applying, run a deliberate concurrency test (fire 10-15 concurrent short requests) and watch `docker stats qwen35` + `free -m` on the box to confirm host memory stays flat instead of climbing — this is the confirmation step the earlier fix lacked. Cost: one `qwen35` recreate (~5-8min reload, same class of change as Entry 115's gpu_util edit) — do during an idle window.

**2. `mem_limit`/`memswap_limit` on `qwen35`, same pattern as `chromadb`'s existing 10g cap — do second, as a backstop not the primary defense.**
`qwen35` currently has NO memory ceiling in compose, unlike `chromadb` (10g/12g) — the one container with real memory discipline isn't the one that caused Entry 116's incident; the one with none is. **Don't guess the number** — once Spark is idle and reachable, run `docker stats qwen35` at steady state (post item 1's fix) and set `mem_limit` to steady-state + ~15-20% headroom (rough starting estimate from the GPU-budget table: ~86GB at gpu_util 0.70, scaled to ~74GB at the current 0.60 → a provisional ~90g ceiling pending real measurement). **Trade-off to flag explicitly:** if this fires, it's a container OOM-kill + ~90s+ reload — messier than item 1's clean rejection. Treat it as the circuit breaker of last resort behind item 1, not a substitute for it. Same deploy mechanics/downtime class as item 1 — can likely be bundled into the same `qwen35` recreate window.

**3. systemd resource protection for `sshd`/`tailscaled`/`docker` — do third, one unit at a time, verify access after each.**
Drop-in override per unit, e.g. `/etc/systemd/system/ssh.service.d/override.conf`:
```
[Service]
MemoryHigh=2G
MemoryMin=512M
OOMScoreAdjust=-900
```
Same pattern for `tailscaled.service` (protects the exact channel we lost reachability through) and `docker.service`. `MemoryMin` needs systemd's cgroup-v2 memory protection to actually apply (verify `system.slice` isn't already capped below what these units request); `OOMScoreAdjust=-900` makes the kernel OOM-killer avoid these processes even without an explicit memory floor. Requires `systemctl daemon-reload` + a restart of each affected unit to take effect immediately (or it applies naturally on next restart/reboot without forcing one now). **Sequencing matters:** do `sshd` first and confirm the current SSH session / a fresh connection survives the restart before touching `tailscaled` or `docker` — this is the exact access path we'd need to recover from if any step here goes wrong. Restarting `docker.service` itself needs `live-restore` confirmed enabled first, or it will affect every running container, not just take a protective config.

**4. `earlyoom` (or tune `systemd-oomd` if already present) — do fourth, as a host-wide tripwire independent of any single container's cgroup.**
dmesg showed **zero** kernel OOM-killer activity during either Entry 116 episode — the box swapped 10-15GB and thrashed for tens of minutes instead of a fast, clean kill. Check `systemctl status systemd-oomd` first (Ubuntu 24.04 may already ship it, possibly just untuned/too lenient by default). If tuning that isn't straightforward, `earlyoom` is simpler and more predictable: install with e.g. `-m 5 -s 5` (trigger at 5% memory / 5% swap free) and use its `--avoid` flag to exclude `sshd`/`tailscaled` from ever being its kill target, directing it instead at the heaviest recently-growing process (in Entry 116, that would have fired well before the 16:05:59 host-down transition, likely producing a contained container restart instead of a 25-minute-plus network outage). This is a genuinely new safety net, not a duplicate of item 2 — it fires on host-wide pressure regardless of whether any individual container's `mem_limit` has been breached, which matters if `qwen35`'s `--ipc host` setup weakens per-container cgroup memory accounting for GPU-related shared memory (worth checking once instrumented).

**5. Converge on ONE concurrency authority instead of two uncoordinated client-side semaphores — structural fix, do last / longest lead time.**
Entry 116's actual trigger was `pipeline/config.yaml`'s `dgx_spark` (concurrency 12, direct) and `spark` (concurrency 32, via the LiteLLM proxy at `llm.troy-davis.com`) never knowing about each other's in-flight requests. Items 1-4 make the *failure mode* graceful; this is the fix for the *cause*. Recommend: fold all traffic through the LiteLLM proxy exclusively (retire or make exceptional the direct `dgx_spark` backend in `pipeline/config.yaml`) and set the proxy's `max_parallel_requests` (or `rpm`/`tpm`) on the spark model deployment to the single agreed-safe number — one ingress path, one enforced ceiling, regardless of how many independent jobs/clients exist upstream of it. Longer lead time because it touches the pipeline's config/routing, not just Spark-side infra — treat as a follow-up conversation with whoever owns `pipeline/config.yaml`, not a same-day change.

**Sequencing confirmed by Troy (2026-08-03, post-recovery from Entry 118): earlyoom first → qwen35 recreate (max-num-seqs + mem_limit) → systemd protection → proxy consolidation (separate thread).**

**Item 4 (earlyoom) EXECUTED 2026-08-03 ~16:28 EDT.** Installed via `apt-get install -y earlyoom` (clean install — neither `earlyoom` nor `systemd-oomd` was present/active on this box beforehand, no conflict to resolve). **Configured non-default thresholds, deliberately deviating from earlyoom's defaults** in `/etc/default/earlyoom`:
```
EARLYOOM_ARGS="-m 5,2 -s 100 -p -r 300 --avoid 'sshd|tailscaled|dockerd|containerd'"
```
**Why non-default:** earlyoom's own docs confirm "both memory and swap must be below minimum for earlyoom to act" (AND-gated). Entry 116's actual failure mode had host RAM collapse to ~100-200MB available while swap sat at ~70%+ free the *entire time* (peak observed swap usage ~11GB/15GB, never neared full) — under earlyoom's default `-s 10`, the swap-side condition would likely never have been satisfied, meaning **default earlyoom would NOT have fired during Entry 116's actual incident.** `-s 100` neutralizes the swap-gating (condition becomes trivially true) so triggering is memory-pressure-only, matching this system's observed failure mode. `-m 5,2`: SIGTERM at 5% available (~6GB) / SIGKILL at 2% (~2.4GB) — calibrated against this system's observed healthy-steady-state baseline (~9.4GB available during Entry 116's pre-incident stable hours, per the Prometheus data), giving real margin against false-positives under normal operation while still catching a runaway well before the catastrophic <200MB state, unlike a naive default `-m 10` (~12.1GB) which would sit ABOVE the normal 9.4GB baseline and misfire constantly under healthy load. `--avoid` protects the exact management-plane daemons (sshd/tailscaled/dockerd/containerd) that were unreachable for 1+ hour during Entry 116/118.
**Verified:** `systemctl status earlyoom` active/running; `ps aux` confirmed the running process has the exact intended argv. journalctl startup log NOT verified (the `claude` user lacks journal-read group membership and `sudo journalctl` requires an interactive TTY — a real but minor gap, not investigated further this session). **Not yet verified: an actual triggered-kill event** — this is unexercised until a real (or deliberately simulated) memory-pressure event occurs. No downtime, no impact to the running docker compose stack.

**Items 1+2 (qwen35 `--max-num-seqs` + `mem_limit`) EXECUTED 2026-08-03 ~16:32-16:37 EDT.** Backed up `/home/claude/docker-compose.yml` → `docker-compose.yml.bak-pre-resilience-20260803` first. Diff applied to the `qwen35` service block:
```diff
       - --speculative-config
...
+    mem_limit: 100g
```
**`--max-num-seqs 8` (revised down from the Entry 117 draft's placeholder of 4):** recalibrated against evidence, not the original guess — production has run c16 cleanly in benchmarks/soaks many times (Entry 080 re-baseline c16 730.5 tok/s), and today's actual crash-point was also 16, so the defect wasn't "16 is inherently unsafe," it was "no ceiling existed to stop it climbing past whatever the safe number is." Chose 8: below today's crash point, and — deliberately — below the pipeline's own configured direct-backend concurrency of 12 (`pipeline/config.yaml` `dgx_spark.concurrency`), so it creates real, felt backpressure/queuing that surfaces the still-open Entry 116 item 5 (dual uncoordinated concurrency configs) to whoever runs the pipeline, rather than silently absorbing it.

**`mem_limit: 100g` — explicitly a backstop, not the primary defense, and its real effectiveness is UNCONFIRMED:** `docker stats qwen35` reported only ~3.5GiB in use pre-change despite the documented GPU budget saying this container legitimately reserves ~74GB at `gpu-memory-utilization 0.60` — strong evidence that cgroup memory accounting is NOT capturing the real unified-memory reservation on this GB10 platform (GPU-driver-owned pages likely aren't attributed to the container's cgroup the way host-side RSS is). Given that, calibrating a tight limit from observed `docker stats` data would have been actively dangerous (would OOM-kill under any real load). Set generously (100g, well above the ~74-86GB legitimate range, well below the 121GB host total) as a "won't hurt if it's a no-op, might help if cgroup limits do bind GPU-unified-memory here" measure — **do not rely on this as the tested/confirmed mechanism; `--max-num-seqs` is the one with verified effect.**

**Verification:** recreate completed cleanly (`docker compose up -d --no-deps qwen35`); vLLM startup log explicitly confirms the flag was parsed: `non-default args: {..., 'max_num_seqs': 8, ...}`. Health flipped to `healthy` at 20:37:16Z (~5min warmup, normal). All other 7 containers (bge-m3 still intentionally stopped) undisturbed, still healthy throughout. Live inference smoke test: first attempt returned empty (20s client timeout too short for a cold first-request Triton JIT compile — not an error); retry with 75s timeout succeeded in 16.3s wall time with a clean 200 OK and normal post-request engine metrics (SpecDecoding 100% acceptance, GPU KV cache 0.0% at idle). No errors/tracebacks in logs.

**Item 3 (systemd protection for the management plane) EXECUTED 2026-08-03 ~16:40-16:45 EDT.** Drop-in overrides, same content on all three units:
```
[Service]
MemoryHigh=2G
MemoryMin=512M
OOMScoreAdjust=-900
```
- **`ssh.service`** (confirmed the real unit name — `sshd.service` doesn't exist on this box): override applied, `daemon-reload` + `restart` — verified via a **fresh** SSH connection succeeding immediately after, and `systemctl show` confirming the values took (`MemoryMin=536870912`, `MemoryHigh=2147483648`, `OOMScoreAdjust=-900`).
- **`tailscaled.service`**: same override, restarted — verified via `tailscale ping spark` succeeding (2ms, direct path via 192.168.10.33 — no longer relayed through "mia," an incidental improvement) immediately after restart.
- **`docker.service`**: same override applied and `daemon-reload` run, but **deliberately NOT restarted now** — `/etc/docker/daemon.json` doesn't exist on this box, meaning `live-restore` is unconfigured (defaults off), so restarting the daemon right now would have bounced all 8 just-recovered containers for no immediate benefit. Config is staged and takes effect on the next natural restart/reboot. Confirmed via `ActiveEnterTimestamp` unchanged (still the original 16:17:10 boot time) and all 8 containers undisturbed at their pre-existing uptimes. **Follow-up worth doing separately: enable `live-restore` in `/etc/docker/daemon.json`** so a future `docker.service` restart (to pick up this protection, or for any other reason) doesn't require full-stack downtime — not done this session, flagging for later.

**Items 1, 2, 3, 4 of the Entry 117 plan are now complete. Item 5 (LiteLLM proxy concurrency consolidation) remains — separate thread with whoever owns `pipeline/config.yaml`, not a Spark-infra change.**

Cross-ref Entry 116, Entry 118, [[spark-outage-2026-08-03]].

## Entry 118 — Total power failure after hard power cycle: no lights, no fans; recovered via full drain-cycle (2026-08-03)

**What happened.** The second outage from Entry 116 (started 17:30:59Z) never self-recovered — still fully unreachable at ~59min, well past the first episode's ~25min self-clear, with no OOM-killer/earlyoom safety net in place (Entry 117 item 4 — not yet implemented) to break the thrash. Troy confirmed nothing was still hitting Spark (the triggering kb-dedup/pipeline work had stopped). Recommended a hard power cycle given: no software path responded at all (SSH full connection timeout, not refused), no recent driver/kernel work so the CLAUDE.md MOK-enrollment reboot hazard didn't apply, and no OOM-killer activity had fired all day (dmesg checked in Entry 116) meaning nothing was going to break the state on its own. **Troy power-cycled it and got NOTHING — no lights at all, no fans.** Complete apparent power loss, not just an unresponsive OS.

**Recon gap discovered mid-incident:** the weekly `spark-recon-cron.sh` (forum/landscape monitoring) has been silently broken for **3 full cycles** (07-18, 07-25, 08-01 all logged `CRIT: claude CLI not found on PATH` in `recon.log` — a cron-environment PATH issue, not investigated further this session, needs a follow-up fix) — so the most recent forum intelligence in this notebook was Entry 113 (2026-07-17), over two weeks stale. Dispatched a live Explore-agent forum check instead of relying on the stale notebook data, given the urgency.

**What the (fresh, 2026-08-03) forum check found — this is a known, unresolved, GROWING NVIDIA-acknowledged hardware pattern, not a config issue:**
- `/t/376039` (721 MHz SM-clock-cap thread): NVIDIA mod AakankshaS (Jul 8) confirmed the constraint is real, not a telemetry artifact — no fix shipped. Two **community-reported, NVIDIA-unendorsed** drain-cycle workarounds: (a) unplug from power, wait a few minutes, reconnect+boot (vaclav.sisl, Jul 10) — theorized as a power-negotiation issue possibly OOM-triggered; (b) unplug, hold power button 10s, reconnect (0rand, Jul 10) — theorizes the PSU enters a safety-lockout mode under resource exhaustion needing a full power-circuit reset, not just a reboot/power-button toggle.
- `/t/376761` (Jul 14): no fan activity after a power event — **PSU swap did NOT fix it** (points to board-level failure, not just PSU) — unresolved, no NVIDIA response.
- `/t/378500` (Jul 29, referencing a blog "Your DGX Spark Is Cooking Itself"): hard power-off under sustained load, unit does NOT auto-restart despite "restart after power loss" BIOS setting — unresolved.
- `/t/378945` (~Aug 2, ONE DAY before this incident): fans stop under headless/heavy load, case gets too hot to touch — thread literally uses "fire hazard" language. **Safety note given to Troy:** don't leave the unit plugged in unattended, and if any burning smell/unusual heat is noticed, don't power-cycle again — leave it unplugged.
- `/t/377733` (Jul 22): XID 13 + segfaults under GPU stress → failing FieldDiag confirmed by NVIDIA (aniculescu) as sufficient alone for RMA approval — "back up your data" and proceed. **FieldDiag (`/opt/nvidia/dgx-spark-fieldiag`) requires the unit to boot** — no documented non-boot RMA diagnostic path was found; standard route is opening a support ticket describing symptoms + troubleshooting done.
- RMA portal: `nvidia.custhelp.com/app/ask` (ticket) or `/app/chat/chat_launch` (live chat). One community report cited RMA turnaround up to ~2 months.

**RESULT — CORRECTION: it came back WITHOUT the drain-cycle being performed.** Troy confirmed he had NOT clicked/toggled the power switch or done the recommended unplug-cycle — the unit was found powered back on. **This changes the diagnostic picture**: this was not "the community drain-cycle workaround fixed it" — it was an unattended, unexplained return from a fully-dark (no lights/fans) state. Possible explanations, none yet confirmed: (a) a BIOS/EC "restart after power loss"-style auto-recovery fired after some internal delay (one forum thread, `/t/378500`, reported this setting NOT working for an identical symptom, but behavior may differ by unit/firmware); (b) an external power event on Troy's side (outlet, power strip, UPS self-test/blip) caused both the original loss and the recovery, independent of any Spark-side PSU defect; (c) the earlier "no lights" observation was momentary/incomplete and the unit was already mid-recovery when checked. **Open question, not yet answered — worth Troy checking:** what is the Spark plugged into (direct outlet vs. power strip vs. UPS), and is there any log/indicator on that upstream device suggesting a power interruption at the relevant time? This matters for whether the standing risk is Spark-hardware-side (PSU/board, per the forum pattern) or external-power-side (worth ruling out before considering RMA). Live SSH verification immediately after discovery it was back on:
- `uptime`: **0 minutes** — genuine cold boot, not a network flap.
- `dpkg --audit` and `dkms status`: both clean — no half-configured packages, no DKMS trap (the CLAUDE.md dist-upgrade/MOK hazard did not apply here — no driver work occurred).
- **GPU clock: 2392 MHz** at 45°C idle — full speed, **NOT** stuck at the 721 MHz cap other units in the forum thread reported. This specific unit does not appear to be exhibiting that symptom post-recovery.
- Memory: 121Gi total, **58Gi available**, swap **5.6MiB used** (essentially empty) — full reset cleared the multi-hour memory pressure from Entry 116 that never released on its own.
- dmesg: no errors/XID/thermal/panic messages around the boot.
- All 8 containers auto-started via `--restart unless-stopped`; `chromadb`/`gliner`/`neo4j`/`ce-service` healthy within 43s, `qwen35`/`qwen3-embed` still warming (normal — cold vLLM start up to ~7min for Triton JIT). Follow-up poll dispatched to confirm `qwen35` reaches `healthy`.
