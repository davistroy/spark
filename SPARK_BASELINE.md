...
| Field | Value |
|-------|-------|
| svd_last_checked_date | 2026-06-23. **NEW WHEEL June 23:** `0.23.1rc1.dev288+gc97e8f99d.d20260622` (+81 commits vs prior dev207), FlashInfer `0.6.13-a671c02e-d20260622` (new build date; same 0.6.13 minor). **Arm C eval target updated from dev207 → dev288.** Last repo commit: June 19 — "Rolled back DeepSeek PR inclusion" (PR #41834 SM12x Triton fallbacks added and reverted same day — still unstable). Prior changes still apply: gpu_memory_utilization default 0.7→0.8; CUDA base `nvidia/cuda:13.0.2-devel-ubuntu24.04`; MiniMax IPC fused path disabled (PR #43410). **PR #279** (DFlash + FlashInfer FP8 KV Cache) still OPEN. Do not combine instanttensor with DFlash (issue #211). |

## Forum Tracking
| Field | Value |
|-------|-------|
| forum_last_checked_date | 2026-06-23 (WebSearch fallback; 719.json returns 403 in remote execution env) |
...
## Watch Items
- **[NEW 2026-06-23 — Entry 086] DGX Spark hard power-off under GPU load (/t/373251).** User on latest OTA ("May 2026" = ~580.x driver, all firmware applied) reports unit completely powers off within ~60s of vLLM stress test, reproducible on demand, zero crash logs captured. Related thread: /t/369457 (DGX-Spark shutdown under high load VLM/GPU Burn, RMA requested). First appearance in recon (thread ~1 week old, age uncertain). Our production unit: 41+ days clean, zero restarts/OOM. Low urgency — monitor; compare firmware versions when forum access available.
...
- **[UPDATED 2026-06-23 — Entry 086] vLLM 0.23.x upgrade eval: Arm C target updated to dev288.** v0.23.0 released 2026-06-15; eugr now at v0.23.1rc1.dev288 (June 22 build, released June 23 at 00:40, +81 commits vs prior dev207). v0.23.0 adds FP4 GEMM for SM120/121 (FlashInfer b12x MoE + CUTLASS-FP4) — the enabling kernel for NVFP4 on single-node DGX Spark. **Arm C:** eugr dev288 (`0.23.1rc1.dev288+gc97e8f99d.d20260622`, FlashInfer `0.6.13-a671c02e-d20260622`) + current FP8 model (pure build upgrade). **Arm D:** NVFP4 (`RedHatAI/Qwen3.6-35B-A3B-NVFP4` or NVIDIA-co-developed checkpoint) + MTP-3; expected +33% c1, −21% c8 vs current FP8+MTP2. DFlash+FP8-KV (PR #279) still OPEN — add as Arm B3 once merged. No explicit SM121/cuDNN graph-corruption text in v0.23.0 release notes; verify via soak test. Migration notes: `--speculative-config` JSON, gpu_mem_util→0.80, validate UMA. Caution: sparkrun issue #164 (FlashInfer in eugr :latest). Sandbox only.
- **[UPDATED 2026-06-22 — Entry 085]** **Qwen3.7 27B/35B open weights NOT released — check Qwen HF org WEEKLY through mid-July.** Three consecutive post-3.6 Qwen releases (3.6-Plus, 3.7-Max, 3.7-Plus) are all API-only, closed-weight. No HF repo under official Qwen org as of June 22. If no release by 2026-07-16, update this item to treat closed-weight-first as working conclusion and shift attention to Qwen4 and new A3B-class entrants (North Mini Code, Nex-N2 Mini). A Qwen3.7-35B-A3B-FP8 would still be a near-drop-in successor when released. Beware HF squat `RscriptSQwen/Qwen3.7-plus` (fake, 2026-06-04).
