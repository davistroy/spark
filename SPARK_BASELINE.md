...
- **[UPDATED 2026-07-04 — Entry 098] vLLM upgrade eval / Arm C: eugr dev764 (`0.23.1rc1.dev764+g54b16d8a9.d20260703`) + FlashInfer 0.6.14 published July 3 — Arm C eval target updated from dev701 to dev764.** Still v0.23.1rc1 base; v0.24.x-based eugr build not yet published. Decision point: schedule Arm C eval against dev701 now, or wait for first v0.24.x-based eugr build (no clear timeline). Dev701 carries: NVFP4 regression fix (#46756 revert), tf5 deprecation, Gemma4-26B recipe, NCCL main branch, Rust frontend, DSV4F recipe (2-node). Prior: Arm C target was dev537 (June 28). Original item below retained for context. v0.24.0 released 2026-06-29T19:41:59Z GitHub / June 30 UTC PyPI (408 commits). Key SM121-adjacent: DFlash+FlashInfer MAINLINED (#43081), EAGLE3 for Qwen3 (#43132), Dynamic SD, CUTLASS FP8 bypass (+20%), MoE-permute pre-alloc (+9–14%), SM120 DSV4 (NOT SM121). No SM121/GB10 text. Correction: Entry 091 "v0.24.0 doesn't exist" was correct at time (June 26 search was ahead of actual June 29 release). eugr current latest: dev537 (`0.23.1rc1.dev537+g6eb63a1da.d20260628`, June 28, 11:45 UTC) — pre-v0.24.0; watch for first v0.24.x-based eugr build. v0.23.0 prior latest. Prior dev448/dev480 key changes carry forward: DeepGEMM switched to `nv_dev` branch; DSV4F (MoE kernel) patch for broken vLLM PR #43008. **New context (Entry 090):** vLLM #43906 (MXFP8 MoE → Marlin fallback on SM121) and #43507 (CUTLASS MoE unavailable SM121 for FP8-Dynamic) confirm SM121 MoE backend coverage gaps for non-standard quant formats — not a production FP8 blocker but add Arm D NVFP4 eval requirement: verify CUTLASS-FP4 MoE dispatch actually lands on SM121 (vs Marlin fallback) during eval. **New context (Entry 091):** PR #41834 (SM12x DSV4F support, OPEN, 116 files) advancing SM121 ecosystem — when merged, eugr will pick up next nightly. v0.23.0 adds FP4 GEMM for SM120/121 (FlashInfer b12x MoE + CUTLASS-FP4) — the enabling kernel for NVFP4 on single-node DGX Spark. **Arm C:** eugr dev520 (`0.23.1rc1.dev520+g9fd00ee00.d20260627`) + current FP8 model (pure build upgrade); fallback pin dev448 (`0.23.1rc1.dev448+ge53a17232.d20260625`, FlashInfer `0.6.13-25dd814e-d20260625`) if dev520 regresses. If daily build cadence continues, consider pinning dev448 rather than chasing latest. **Arm D:** NVFP4 (`RedHatAI/Qwen3.6-35B-A3B-NVFP4` or NVIDIA-co-developed checkpoint) + MTP-3; expected +33% c1, −21% c8 vs current FP8+MTP2; add CUTLASS-FP4 dispatch verification to eval protocol. DFlash+FP8-KV (PR #279) still OPEN — add as Arm B3 once merged. No explicit SM121/cuDNN graph-corruption text in v0.23.0 release notes; verify via soak test. Migration notes: `--speculative-config` JSON, gpu_mem_util→0.80, validate UMA. Caution: sparkrun issue #164 (FlashInfer in eugr :latest). Sandbox only.
- **[UPDATED 2026-07-06 — Entry 100]** **Qwen3.7 27B/35B open weights NOT released — 48 days past Qwen3.7-Max API launch (May 19). July 16 deadline in 10 days.** Three consecutive post-3.6 Qwen releases (3.6-Plus, 3.7-Max, 3.7-Plus) are all API-only, closed-weight. No HF repo under official Qwen org. InsiderLLM analysis: "If we get to mid-July without one, the gap is widening." Probability of release before July 16 declining. Holo-3.1-35B-A3B open weights confirmed (Entry 099) but it is a VLM — not a general LLM successor. Ornith-1.0-35B-FP8 remains best confirmed open-weight A3B general LLM alternative (MTP pre-screen required). A Qwen3.7-35B-A3B-FP8 would still be a near-drop-in successor when released. Beware HF squat `RscriptSQuen/Qwen3.7-plus` (fake). **If absent July 16, shift to closed-weight-first conclusion.**
