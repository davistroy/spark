...
- **Ethernet static IP configured:** enP7s7 set to <spark-lan-ip>/24 (manual, gateway <gateway-ip>). WiFi stays on <spark-lan-ip>. NM connection files stored in both `/run/NetworkManager/system-connections/` (runtime) and `/etc/NetworkManager/system-connections/` (persistent). NM on this system reads from `/run/` first — always write there AND `/etc/`.
