...
| Field | Value |
|-------|-------|
| svd_last_checked_date | 2026-06-25 (Entry 088). No new wheel or repo commit since dev309 (June 23, ~11:30 UTC). Latest: `0.23.1rc1.dev309+g901a3b091.d20260623`, FlashInfer `0.6.13-b3baedbb-d20260623`. **Arm C eval target remains dev309.** PR #279 (DFlash + FlashInfer FP8 KV Cache) still OPEN. All prior notes unchanged: gpu_memory_utilization default 0.7→0.8; CUDA base `nvidia/cuda:13.0.2-devel-ubuntu24.04`; MiniMax IPC fused path disabled (PR #43410); do not combine instanttensor with DFlash (issue #211). |

## Forum Tracking
| Field | Value |
|-------|-------|
| forum_last_checked_date | 2026-06-25 (Entry 088; WebSearch fallback; 719.json returns 403 in remote execution env) |
...
## Watch Items
- **[UPDATED 2026-06-25 — Entry 088] DGX Spark hardware-reliability cluster — now 4 threads.** /t/373251 (hard power-off within 60s of vLLM stress test, latest OTA/580.x, reproducible on demand, zero crash capture); /t/362483 "System powers off" (older); /t/372089 "DGX Spark Failure – Unable to Power On" (Entry 087); **/t/372486 "DGX Spark GB10 – Asus GX10 – GPU becomes inoperable"** (newly surfaced Entry 088; different symptom — GPU enters non-functional state after repeated workloads, Asus-specific). Also /t/369457 (RMA requested). No NVIDIA resolution found. Our production unit: 43+ days clean, zero restarts/OOM. Low urgency — monitor; check for NVIDIA acknowledgment when forum access available.
...
- **[UPDATED 2026-06-25 — Entry 088]** **Qwen3.7 27B/35B open weights NOT released — June window closing; check weekly through mid-July.** Three consecutive post-3.6 Qwen releases (3.6-Plus, 3.7-Max, 3.7-Plus) are all API-only, closed-weight. No HF repo under official Qwen org as of June 25 (confirmed Entry 088 via InsiderLLM analysis + WebSearch). Original projection (3.5→3.6 lag = 51-59 days) pointed to June 6-14; June 25 is >5 weeks past 3.7-Max API launch with no open weights. Realistic window now late June through mid-July, probability declining. If no release by 2026-07-16, update this item to treat closed-weight-first as working conclusion and shift attention to Qwen4 and new A3B-class entrants (North Mini Code, Nex-N2 Mini). A Qwen3.7-35B-A3B-FP8 would still be a near-drop-in successor when released. Beware HF squat `RscriptSQwen/Qwen3.7-plus` (fake, 2026-06-04). **Next check: July 3.**
