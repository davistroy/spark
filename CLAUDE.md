...
- **NVFP4 (modelopt-FP4) does NOT load on the current cu132 build (v0.19.1rc1.dev219) — Entry 083, 2026-06-30:** `nvidia/Qwen3.6-35B-A3B-NVFP4` dies at weight-load with `KeyError: 'layers.0.mlp.experts.w2_input_scale'` (`vllm/model_executor/models/qwen3_5.py:407`) — the loader has no mapping for the NVFP4 MoE per-expert input-scale tensors. **NOT a kernel/CUDA-graph/#2776 issue and NOT a Marlin hang** — a pure weight-schema gap that no runtime flag fixes. **The NVFP4 eval is therefore coupled to the v0.23.x/v0.24.0 build upgrade (Arm C); there is no current-build shortcut.** Model IS cached (22 GB, `quant_method: modelopt`, arch `Qwen3_5MoeForConditionalGeneration`). External Arena/forum NVFP4 wins (Poveda 118.91 tok/s c1, +78% vs prod) all ran on newer builds — consistent. Re-attempt only on a v0.23.x image. The B1 probe (single ~12-min idle prod-down window, zero GPU risk) was the correct cheap test before committing to the multi-hour build. **Distinct from the AWQ-INT4 rule below** — that was a CUDA-graph hang; this is a load-time schema gap.
