...
- **3.0 download ✅** — `nvidia/Qwen3.6-35B-A3B-NVFP4` fully cached (17 files / 22 GB blobs, 3 safetensors shards). `quant_method: modelopt` (NVFP4), arch `Qwen3_5MoeForConditionalGeneration`, KV not baked in.
- **B1 (NVFP4 on CURRENT build) ✅ RAN → FAILED FAST (decisive, U1 RESOLVED).** Stopped prod qwen35 (idle 0%/11.8W), removed stopped container (name-conflict — eval compose `up` needs the prod container *removed*, not just `stop`ped — harness usage note is incomplete; documented for next time), launched `nvfp4_curbuild`. Resolved command verified single-variable (NVFP4 model + `--kv-cache-dtype fp8` + MTP=3, else == prod). **EngineCore died in ~20 s during weight load:** `KeyError: 'layers.0.mlp.experts.w2_input_scale'` at `vllm/model_executor/models/qwen3_5.py:407 load_weights`. **Root cause: the current build's `qwen3_5.py` loader has no mapping for the NVFP4 MoE per-expert input-scale tensors** — i.e. v0.19.1rc1.dev219 cannot parse the modelopt-FP4 MoE checkpoint. **NOT a kernel/CUDA-graph issue (#2776) and NOT a Marlin hang** — a weight-schema gap that no runtime flag fixes. **Conclusion: NVFP4 is genuinely coupled to Arm C (v0.23.x build); there is no current-build shortcut.** B1 cost: one ~12-min production-down window, no GPU/kernel risk realized.
- **B2 (obtain v0.23.x image) — REQUIRED, multi-hour build window.** Not pullable (no eugr DockerHub image); local `/home/claude/spark-vllm-docker` is stale (Apr 30) → needs `git pull` + source/wheels build (FlashInfer + vLLM for sm_121). **Deferred to a scheduled window** (see plan §3.2). B3/B4/B5 follow on the built image.
- **B6 restore production ✅** — `restore_production.sh` brought prod back: `/health` 200 at 2026-06-30T17:17:38Z; served `spark-llm` root `Qwen/Qwen3.6-35B-A3B-FP8`, `speculative-config {"method":"mtp","num_speculative_tokens":2}`, no `--quantization`/`--kv-cache-dtype`, maxlen 131072 — **exact Entry 073 production config.** All 8 containers healthy. Total prod-down window for B1 ≈ 12 min (idle, no consumer impact observed). Swap re-verify (2.2) still pending a quiet window.

#### Decision / next steps
- **NVFP4 eval BLOCKED on B2 (v0.23.x build), deferred to a user-scheduled window** — build is multi-hour + memory-pressure (run prod-down or limited BUILD_JOBS). Recorded as a WATCH item in SPARK_BASELINE + a verified rule in CLAUDE.md. ADR-0001 stays Proposed.
- **davistroy follow-ups:** 1.5 (Windows-laptop host-key re-pin) + 2.1 (`usermod -aG systemd-journal,adm claude`).

#### Verification (this session)
CVE: `ssh … 'sudo systemctl is-active ssh'`=active; fresh connect OK; ed25519 fp ≠ factory `ah+SGtSN…` ✓. Builder prune: 53.42 GB reclaimed ✓. B1: resolved command verified single-variable; failure reproducible (`KeyError w2_input_scale`) ✓.
