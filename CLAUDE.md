...
- **`apt dist-upgrade` REMOVES the running kernel's nvidia module (2026-06-15, Entry 077 — verified via `apt -s`).** A kernel bump through dist-upgrade (e.g. 6.17.0-1014→1021) installs the new kernel but (a) does NOT install the new kernel's `linux-modules-nvidia-580-open-{ver}` and (b) REMOVES the *currently-running* kernel's module. Naive `dist-upgrade && reboot` → BOTH kernels lack a working nvidia module → GPU dead on boot AND GRUB fallback to the old kernel also broken → physical recovery only. **Before any reboot:** `sudo apt install linux-modules-nvidia-580-open-{new-kernel}` AND re-install the current kernel's module so the GRUB fallback stays bootable; verify both modules on disk first. Stay on 580.x (590 = GB10 UMA leak, unsupported).
