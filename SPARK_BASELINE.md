...
| Field | Value |
|-------|-------|
| svd_last_checked_date | 2026-06-24. **NEW WHEEL June 23 (~11:30 UTC, post-Entry 086):** `0.23.1rc1.dev309+g901a3b091.d20260623` (+21 commits vs dev288 = +102 total vs dev207), FlashInfer `0.6.13-b3baedbb-d20260623` (new build, same 0.6.13 minor). **Arm C eval target updated dev288 → dev309.** Last repo commit: June 23 — "Updated README with DeepSeek V4 Flash support." Prior changes still apply: gpu_memory_utilization default 0.7→0.8; CUDA base `nvidia/cuda:13.0.2-devel-ubuntu24.04`; MiniMax IPC fused path disabled (PR #43410). **PR #279** (DFlash + FlashInfer FP8 KV Cache) still OPEN. Do not combine instanttensor with DFlash (issue #211). |

## Forum Tracking
| Field | Value |
|-------|-------|
| forum_last_checked_date | 2026-06-24 (WebSearch fallback; 719.json returns 403 in remote execution env) |
...
## Watch Items
- **[UPDATED 2026-06-24 — Entry 087] DGX Spark power-instability cluster (/t/373251, /t/362483, /t/372089).** Three distinct threads now: /t/373251 (hard power-off within 60s of vLLM stress test, latest OTA/580.x, reproducible on demand, zero crash capture); /t/362483 "System powers off" (older thread); /t/372089 "DGX Spark Failure – Unable to Power On" (date unclear, newly surfaced Entry 087). Also /t/369457 (RMA requested). No NVIDIA resolution found. Our production unit: 42+ days clean, zero restarts/OOM. Low urgency — monitor; check for NVIDIA acknowledgment when forum access available.
...
- **[UPDATED 2026-06-24 — Entry 087] vLLM 0.23.x upgrade eval: Arm C target updated to dev309.** v0.23.0 released 2026-06-15; eugr now at v0.23.1rc1.dev309 (June 23 build, released June 23 ~11:30 UTC, +21 commits vs dev288 = +102 total vs dev207). v0.23.0 adds FP4 GEMM for SM120/121 (FlashInfer b12x MoE + CUTLASS-FP4) — the enabling kernel for NVFP4 on single-node DGX Spark. **Arm C:** eugr dev309 (`0.23.1rc1.dev309+g901a3b091.d20260623`, FlashInfer `0.6.13-b3baedbb-d20260623`) + current FP8 model (pure build upgrade). **Arm D:** NVFP4 (`RedHatAI/Qwen3.6-35B-A3B-NVFP4` or NVIDIA-co-developed checkpoint) + MTP-3; expected +33% c1, −21% c8 vs current FP8+MTP2. DFlash+FP8-KV (PR #279) still OPEN — add as Arm B3 once merged. No explicit SM121/cuDNN graph-corruption text in v0.23.0 release notes; verify via soak test. Migration notes: `--speculative-config` JSON, gpu_mem_util→0.80, validate UMA. Caution: sparkrun issue #164 (FlashInfer in eugr :latest). Sandbox only.
