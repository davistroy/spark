...
| Field | Value |
|-------|-------|
| svd_last_checked_date | 2026-06-26 (Entry 089). **New wheel published 2026-06-25T20:38Z: `0.23.1rc1.dev448+ge53a17232.d20260625`** (+139 commits vs dev309 in 2-day window), FlashInfer `0.6.13-25dd814e-d20260625`. **Arm C eval target updated dev309 → dev448.** Key commits: (1) DeepGEMM switched to `nv_dev` branch; (2) DSV4F (MoE kernel) patch applied for broken vLLM PR #43008 — relevant to Qwen3.6-35B-A3B MoE dispatch on SM121; (3) NVFP4 recipe kv-cache-dtype reverted to fp8 due to vLLM bugs (not relevant to BF16 KV FP8 config). PR #279 (DFlash + FlashInfer FP8 KV Cache) still OPEN (no activity since June 12). All other prior notes unchanged: gpu_memory_utilization default 0.7→0.8; CUDA base `nvidia/cuda:13.0.2-devel-ubuntu24.04`; MiniMax IPC fused path disabled (PR #43410); do not combine instanttensor with DFlash (issue #211). |

## Forum Tracking
| Field | Value |
|-------|-------|
| forum_last_checked_date | 2026-06-26 (Entry 089; WebSearch fallback; 719.json returns 403 in remote execution env) |
...
- **[UPDATED 2026-06-26 — Entry 089] vLLM 0.23.x upgrade eval: Arm C target updated to dev448.** v0.23.0 released 2026-06-15; eugr now at v0.23.1rc1.dev448 (June 25, 20:38 UTC, +139 commits vs dev309 in 2-day window). Key dev309→dev448 changes: DeepGEMM switched to `nv_dev` branch; DSV4F (MoE kernel) patch applied for broken vLLM PR #43008 — relevant to Qwen3.6-35B-A3B MoE dispatch on SM121. v0.23.0 adds FP4 GEMM for SM120/121 (FlashInfer b12x MoE + CUTLASS-FP4) — the enabling kernel for NVFP4 on single-node DGX Spark. **Arm C:** eugr dev448 (`0.23.1rc1.dev448+ge53a17232.d20260625`, FlashInfer `0.6.13-25dd814e-d20260625`) + current FP8 model (pure build upgrade). **Arm D:** NVFP4 (`RedHatAI/Qwen3.6-35B-A3B-NVFP4` or NVIDIA-co-developed checkpoint) + MTP-3; expected +33% c1, −21% c8 vs current FP8+MTP2. DFlash+FP8-KV (PR #279) still OPEN — add as Arm B3 once merged. No explicit SM121/cuDNN graph-corruption text in v0.23.0 release notes; verify via soak test. Migration notes: `--speculative-config` JSON, gpu_mem_util→0.80, validate UMA. Caution: sparkrun issue #164 (FlashInfer in eugr :latest). Sandbox only.
