...
| vllm_latest_observed | v0.23.0 (2026-06-15, WORTH WATCHING). 408 commits / 200 contributors. Highlights: DeepSeek-V4 hardening, Model Runner V2 default for Llama/Mistral dense, Gemma 4 Unified encoder-free, multi-tier KV cache offloading, "breakable CUDA graphs," prefix-cache corruption remedies, Rust frontend expansion. **v0.23.0 adds FP4 GEMM for SM120/121** (FlashInfer b12x MoE + CUTLASS-FP4 MoE backend) — this is the enabling kernel for NVFP4 on single-node DGX Spark. **No explicit SM121/GB10 text in release notes** — prior belief that cuDNN graph-corruption bug is fixed in v0.23.0 not confirmed from release notes (may still be true; verify via soak test). DeepGEMM SM12x still blocked (#41063 open). Gemma4 PRs #39138 AND #40099 BOTH still open. **This is the target version for upgrade eval** (eugr at 0.23.1rc1.dev129 as of 2026-06-17 — +76 commits from dev53). Prior: v0.22.0 (2026-05-29) had SM121 kernel work (#41215 per-tensor FP8 CUTLASS, #40082 FlashInfer MoE SM120/121, #40717 Blackwell GDN prefill). |
| qwen_current_model | Qwen/Qwen3.6-35B-A3B (adopted 2026-04-23) |
| gemma4_pr_status | #39138 OPEN (active merge conflict flagged 2026-06-15, last activity 2026-06-16; re-confirmed 2026-06-19) / #40099 OPEN (awaiting code-owner review, last activity 2026-06-09; re-confirmed 2026-06-19). Both still required; unmerged in v0.23.0. |

## spark-vllm-docker Tracking
| Field | Value |
|-------|-------|
| svd_last_checked_date | 2026-06-19 (no new wheels since 2026-06-17: vLLM **0.23.1rc1.dev129+g2a47a9ff0.d20260617**, FlashInfer **0.6.13-d8f1dcbd-d20260617** unchanged. Recent recipe changes: **gpu_memory_utilization default raised 0.7→0.8** in single-node and two-node recipes; CUDA base → `nvidia/cuda:13.0.2-devel-ubuntu24.04`; MiniMax QK RMSNorm CUDA IPC fused path disabled (patches vLLM PR #43410). **DFlash + FlashInfer FP8 KV Cache recipe (PR #279) still OPEN** — not yet in wheels. instanttensor experimental load-format added (faster than fastsafetensors); issue #211 documents instanttensor+DFlash crash — do not combine.) |

## Forum Tracking
| Field | Value |
|-------|-------|
| forum_last_checked_date | 2026-06-19 (WebSearch fallback; 719.json returns 403 in remote execution env) |
